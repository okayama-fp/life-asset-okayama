...
以下の最新ニュースを踏まえ、個人の資産形成・節税・家計管理に役立つFP専門ブログ記事を書いてください。

【本日のニュース】
{news_text}

【記事の要件】
- 文字数：3000〜5000文字（必ず守ること）
- 対象読者：資産形成に興味がある20〜50代
- 文体：親しみやすく、専門的な内容をわかりやすく解説
- 構成：
  ① 導入（読者の悩みに共感、今日のニュースとの関連）
  ② 本文（見出し3〜4つ、具体的な数字・事例を含める）
  ③ まとめ（読者へのアクション提案）
- SEO：タイトルに「{YEAR_MONTH}」か具体的なキーワードを含める
- ニュースの内容をFP視点で家計・資産形成に結びつけること
...
<div style="background:linear-gradient(135deg,#fffbeb,#fef3c7);border-top:2px solid #fde68a;padding:40px 24px;text-align:center">
  <div style="max-width:640px;margin:0 auto">
    ...
    <a href="https://note.com/lifeassetpartners" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;gap:8px;background:#000;color:#fff;font-size:0.9rem;font-weight:700;padding:12px 28px;border-radius:8px;text-decoration:none">
      📓 noteマガジンを見る
    </a>
  </div>
</div>
