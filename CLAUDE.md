...
> ⚠️ **絶対禁止（永久）**: `髙月`・`高月`・`義照`・`yoshiteru`・`Takatsuki`・`fp-yoshiteru` など代表者の実名・個人を特定できる文字列は一切コードや記事・ファイル名・alt属性に使用しない。FP表記は「ライフアセットオフィス FP」のみ使用する。画像は `fp-avatar.png` を使用する。
