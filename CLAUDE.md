...
不明な場合はSonnet5をデフォルトとし、タスクの複雑さ・影響範囲が大きいと判断した場合のみOpus4.8へ格上げする。この選定はコスト効率を最優先し、逐一確認は行わない。

### トークン経済性ルール（2026-07-09 追加）
モデル単価の最適化だけでは経済的にならない（単価×使用量がコストのため）。以下も総監督が都度判断し、確認なしで実行する。
