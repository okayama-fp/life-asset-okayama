...
### 家族の個人資産情報の取り扱い（祖母の土地資産等・2026-08-09 追加・厳守）

対象: 祖母の土地資産をはじめ、ユーザーの家族に関する実データ（氏名、住所、地番・地目、面積、評価額、家族構成、続柄、金融資産残高など個人・世帯を特定できる情報）。相続税の個別試算・資産承継プラン検討のためにチャットやファイルで提供される想定。

1. **このリポジトリ（公開GitHub Pages）には実データを一切書き込まない**。ページ本体・ブログ記事・コミットメッセージ・PR本文・Issue・ファイル名・alt属性・コード中のコメント、いずれにも記載しない。
2. チャットやファイルで実データの提供を受けた場合は、リポジトリ直下の `private/` ディレクトリ（`.gitignore` 登録済み・追跡対象外）にのみ保存してよい。それ以外の場所には保存しない。
3. 相続税シミュレーション結果や試算例を記事・ページ化する場合は、**数値を丸める／氏名や地名を仮名化する等、個人を特定できない形に必ず加工してから**使用する。個別の実データによる試算は `private/` 内で完結させ、公開用の汎用ツール（`souzoku-sim.html`）や記事には反映しない。
4. `git push` / PR作成の前に、差分（`git diff` / `git status`）に実データ（氏名・住所・地番・具体的な金額等）が含まれていないか必ず確認する。含まれている疑いがある場合は push・コミットを中断し、ユーザーに報告する。
5. 万一実データを誤ってコミット・pushしてしまった場合は直ちに作業を停止してユーザーに報告する。履歴からの削除（force push等）はユーザーの明示的な許可なしに行わない。
6. 祖母の資産データを含め、家族の個人資産情報を外部API・外部サービス・第三者ツールに送信しない（既存の「外部API通信は事前にユーザーへ確認・許可」ルールを適用）。
